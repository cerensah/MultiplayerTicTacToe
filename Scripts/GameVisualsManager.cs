...
public class GameVisualsManager : NetworkBehaviour
{
    ...
    private void GameManager_OnGameWin(object sender, GameManager.OnGameWinEventArgs e){
        ...
        //we only can spawn on server so if not server break & not instantiate
    ...
    private void SpawnObjectRpc(int x, int y, GameManager.PlayerType playerType){
        ...
        spawnedCrossTransform.GetComponent<NetworkObject>().Spawn(true); //this object should be spawned across network on all clients
        visualGameObjectList.Add(spawnedCrossTransform.gameObject);
    }

    private Vector2 GetGridWorldPosition(int x , int y){
        return new Vector2(-GRID_SIZE + GRID_SIZE * x, -GRID_SIZE + y * GRID_SIZE);
    }
}
